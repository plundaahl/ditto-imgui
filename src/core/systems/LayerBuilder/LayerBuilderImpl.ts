...
enum LayerBuilderState {
    BUILD_MODE,
    RENDER_MODE,
}

export class LayerBuilderImpl implements LayerBuilder {
    ...
    private state: LayerBuilderState = LayerBuilderState.BUILD_MODE;
    ...
    constructor() {
        ...
        this.bringCurrentLayerToFront = this.bringCurrentLayerToFront.bind(this);
    ...
    beginLayer(key: string): void {
        if (this.state !== LayerBuilderState.BUILD_MODE) {
    ...
    onPostRender(): void {
        this.state = LayerBuilderState.BUILD_MODE;
    }
